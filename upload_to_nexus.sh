...
#mvn command to upload the csar in nexus
mvn -X -V -B deploy:deploy-file -Durl=${repositoryUrl} -DrepositoryId=nexus -DgroupId=${packageType} -DartifactId=${package} -Dversion=${version} -DgeneratePom=true -Dpackaging=tar.gz -Dfile=${fileName}
